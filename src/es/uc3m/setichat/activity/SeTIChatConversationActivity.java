...
public class SeTIChatConversationActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.datos = (ContactsToChat) getIntent().getExtras().getSerializable("datos");
		Log.d("conversation", "datos recibidos serverId: " + datos.getServerId() + ", destinatario: " + datos.getDestinatario());
		this.mensaje = new MensajeChat();
		this.mensaje.setEncrypted("false");
		this.mensaje.setIdDestination("105279828.106279828");
		//this.mensaje.setIdSource(this.datos.getServerId());
		this.mensaje.setIdSource(datos.getServerId());  
		this.mensaje.setSigned("false");
		this.mensaje.setType("4");
		...
		responseReceiver = new BroadcastReceiver() {
		    ...
		    public void onReceive(Context context, Intent intent) {
		      //do something based on the intent's action
				...
				//CharSequence texto = (String)bundle.get("message");
		    	MensajeResponse response = (MensajeResponse) intent.getSerializableExtra("mensajeResponse");
	    		Toast toast = Toast.makeText(context1, "\n"+response.getResponseMessage(), duration);
	    		toast.show();
				//text.append("\n"+response.getIdMessage());
		    }
		  };
		  
	IntentFilter responseMessageFilter = new IntentFilter();
	responseMessageFilter.addAction("es.uc3m.SeTIChat.CHAT_OK");
	registerReceiver(responseReceiver, responseMessageFilter);
	...
	chatMessageReceiver = new BroadcastReceiver() {
	    ...
	    public void onReceive(Context context, Intent intent) {
	      //do something based on the intent's action
			...
			//Bundle bundle = intent.getExtras();
			//CharSequence texto = (String)bundle.get("message");
	    	MensajeChat response = (MensajeChat) intent.getSerializableExtra("mensajeResponse");
//    		Toast toast = Toast.makeText(context1, "\n"+response.getResponseMessage(), duration);
//    		toast.show();
			text.append("\n"+response.getMensaje());
	    }
		...
		chatMessageFilter.addAction("es.uc3m.SeTIChat.MENSAJE_CHAT");
	...
	public View conversationView(int index) {
		...
		send.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				mensaje.setMensaje(edit.getText().toString());
				mService.sendMessage(generadorXML.generarXMLChat(mensaje));
				// Refresh textview
